...
10. 在LocoMode模式下，按L1+B进入**乒乓球**模式 — 机器人将追踪并击打乒乓球
    - 球位置从仿真中自动追踪
    - 按R1+A返回行走模式（通过技能冷却过渡）
    - 按Start返回位控模式
    - 按L3或Select进入阻尼保护模式
---
## 5. 真机操作说明
...
- **仿真环境**：球位置自动从场景中提取
- **实机环境**：当前使用**静态回退位置**
  - 默认球位置：`[3.5, -0.2, 1.0]`（前方3.5米，左侧0.2米，高度1.0米）
  - **待办**：集成基于视觉的球追踪系统用于生产部署
- **状态转换**：退出乒乓球模式时，机器人通过 `SkillCooldown` 过渡以保持稳定性
