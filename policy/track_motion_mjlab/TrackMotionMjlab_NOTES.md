# TrackMotionMjlab 实现笔记

日期：2026-04-20

目的：记录在当前框架下新增 `TrackMotionMjlab` 状态（sim2sim）时的实现思路、关键改动和参考来源，便于复现与维护。

**概览**
- 本次实现在 `policy/track_motion_mjlab/` 下新增了状态实现、配置与模型：
  - 状态代码：[policy/track_motion_mjlab/TrackMotionMjlab.py](policy/track_motion_mjlab/TrackMotionMjlab.py)
  - 配置文件：[policy/track_motion_mjlab/config/TrackMotionMjlab.yaml](policy/track_motion_mjlab/config/TrackMotionMjlab.yaml)
  - ONNX 模型：位于 [policy/track_motion_mjlab/model/](policy/track_motion_mjlab/model/)
...
- 使用的公共类型与枚举：
  ...
  - FSM 集成点：已在 [FSM/FSM.py](FSM/FSM.py) 中 import 并实例化 `TrackMotionMjlab`（见 `self.track_motion_mjlab_policy = TrackMotionMjlab(...)`）

**关键实现要点（按模块）**
- TrackMotionMjlab 类（policy/track_motion_mjlab/TrackMotionMjlab.py）
  - 结构上沿用了 `TableTennis` 的 FSMState 模板：初始化 config、构造 `mj_joint_names` 与 `train_joint_names`、创建 `mj_to_train` / `train_to_mj` 重排索引。
  - 配置读取：优先读取 `policy/track_motion_mjlab/config/TrackMotionMjlab.yaml`，并通过 `_resolve_path` 支持相对/绝对路径与 PROJECT_ROOT 回退。
...
- 配置文件（policy/track_motion_mjlab/config/TrackMotionMjlab.yaml）
...
- FSM 集成（FSM/FSM.py）
  - 将 `TrackMotionMjlab` import 到 FSM，构造时传入 `state_cmd` 与 `policy_output`，并在 `get_next_policy` 中处理 `FSMStateName.SKILL_TRACK_MOTION_MJLAB` 的切换逻辑。

**我如何从其他文件/状态借鉴并修改得到 TrackMotionMjlab**
- 复制/对比：从 `policy/table_tennis/TableTennis.py` 复制 FSMState 模板（init / _load_policy / _build_obs / run / exit / checkChange），逐行对比并替换：
  ...
  - 观察项（term_dims/term_order）根据 track_motion_mjlab 任务调整（包含 racket 相关项），并相应修改 `_build_obs()` 的内容与默认命令读取。
  - ONNX 读写细节：TableTennis 假定 single-input single-output；TrackMotionMjlab 支持多个输入名（`obs`, `time_step` 等），并对 inputs/outputs 名称、shape 做额外检查与兼容处理。
...
**快速测试建议**
- 在本地 MuJoCo 仿真或 sim2sim 环境中，把 FSM 切换到 `SKILL_TRACK_MOTION_MJLAB`，观察：
  - 是否能成功加载 ONNX（日志中会打印初始化信息或错误原因）
  - `policy_output.actions` 是否按期望输出（非默认角），kps/kds 是否被正确映射
  - 若使用外部 data，请确认 `policy/track_motion_mjlab/model/policy_*.onnx.data` 可访问

**相关文件索引（便于复查）**
- [policy/track_motion_mjlab/TrackMotionMjlab.py](policy/track_motion_mjlab/TrackMotionMjlab.py)
- [policy/track_motion_mjlab/config/TrackMotionMjlab.yaml](policy/track_motion_mjlab/config/TrackMotionMjlab.yaml)
- [policy/track_motion_mjlab/model/](policy/track_motion_mjlab/model/)
- [policy/table_tennis/TableTennis.py](policy/table_tennis/TableTennis.py)
